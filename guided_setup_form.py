...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 22 12:43:56 2025
...
def guided_setup_form(params_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if submitted:
        ...
        ps["MEMBERSHIP_FEE"] = float(member_fee)
        ps["MEMBERSHIP_PRICE"] = float(member_fee)  # for back-compat

        ps["OWNER_DRAW"] = int(owner_draw)

        ps["EVENTS_PER_MONTH"] = int(events_per_month)
        ps["EVENT_PRICE"] = float(event_price)
        ps["EVENT_TICKET_PRICE"] = float(event_price)
        ps["EVENT_SIZE"] = int(event_size)

        ps["CLAY_PRICE_PER_LB"] = float(clay_price)
        ps["FIRING_FEE_PER_LB"] = float(firing_fee)
